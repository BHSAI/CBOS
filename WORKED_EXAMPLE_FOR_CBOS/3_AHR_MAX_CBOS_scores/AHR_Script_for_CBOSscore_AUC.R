...
#######################
# Note: The code used to process chem vs chem similarity matrix table and split them into reference set and query/test set is given in commented lines
# at the end below.
# For ease/reproducing the result, commented this part and uploading the .RData file used in our run.
#######################
...
#######################
# Note: To create the 1000 random reference set of same size as true reference set, the following 
#  code (in commented lines) was used.
# For ease/reproducing the result, commented this part and uploading the random data used in our run.
#######################
#-----------------
# set.seed(123)
# RAND_bb3 <- data.frame(row.names =1:TR_COUNT)
# for (i in 1:1000){
#     bb<-data.frame(as.character(sample(TARG_ACTall_processed$CID,TR_COUNT)),stringsAsFactors = FALSE)
#     colnames(bb) <-i
#     RAND_bb3 <- data.frame(RAND_bb3,data.frame(bb),stringsAsFactors = FALSE)
# }
# FILEOUT1 <- paste(nTARGET,"_",TARG,"_all_RANDLIST1000.RData",sep="")
# save(RAND_bb3,file = FILEOUT1)
#---------------- 
load("./5_AHR_all_RANDLIST1000.RData")
...
TARG_metric <- data.frame(TARG,CBOSROCAUC,CBOS_e10,nACTIV,nINACTIV)
write.table(TARG_metric,"./AHR_CBOS_perf_metrics.txt", sep = "\t",row.names=FALSE, append = TRUE)
########################################### 
rm(list=ls())
...
#######################
#-----------------------------
# FILEIN <- paste(TARG,"all", "_MATRIX_ROCS_merge.txt",sep="")
# # Get 3D d-d matrix 
# TARG3d_DD <- read.delim(FILEIN,header=TRUE,sep="\t", stringsAsFactors = FALSE)
# TARG3d_DD$Name_1 <- sub("^","X",TARG3d_DD$Name_1)
# row.names(TARG3d_DD) <- TARG3d_DD[,1]
# TARG3d_DD <- TARG3d_DD[,-1]
#****
# # get TARG activity data
# TARG_ACTall <- read.delim(TAR_ACTANNOT,header=TRUE,sep="\t", stringsAsFactors = FALSE)
# TARG_ACTall$CID <- sub("^","X",TARG_ACTall$CID)
# TARG_ACTall_processed <- TARG_ACTall[TARG_ACTall$CID %in% row.names(TARG3d_DD),]
#**** 
# # Get actives - split into TR and Q -get count of ACTIVES
# TARG_ACTonly <- TARG_ACTall_processed[TARG_ACTall_processed$ACT_AN=="Active",]
# TARG_ACT_SAMP <- sample(TARG_ACTonly$CID, (nrow(TARG_ACTonly)/2),replace = FALSE,prob = NULL)
# TR_COUNT <- length(TARG_ACT_SAMP)
# TARG_INACTonly <- TARG_ACTall_processed[TARG_ACTall_processed$ACT_AN=="Inactive",]

# TARG_TR1 <- TARG_ACTonly[TARG_ACTonly$CID %in% TARG_ACT_SAMP,]
# TARG_Q <- TARG_ACTall_processed[!(TARG_ACTall_processed$CID %in% TARG_ACT_SAMP), ]
#--------------------------
###########################################
